...
def tiny_raygen(params: TinyLaunchParams):
    ...
    color = hit_color
    if payload.hit == wp.uint32(0):
        scale = 2.5 / wp.max(-ray_direction[2], 0.001)
        x = ray_direction[0] * scale
        y = ray_direction[1] * scale
        ...
        color = wp.vec3(
            ...
            wp.pow(display_sky[1], 2.2),
            wp.pow(display_sky[2], 2.2),
        )
